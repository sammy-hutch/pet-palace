...
export const useShopPurchaseActions = () => {
                ...
                [
                    {
                        ...
                        style: "cancel",
                        onPress: () => {
                            console.log("Final purchase confirmation cancelled.");
                            resolve(false);
                        }
                    },
                    {
    ...
    const handlePurchase = async (item: PurchasableItem): Promise<boolean> => {
        ...
        if (itemType === 'buyable_cats') {
                ...
                Alert.alert(
                    ...
                    [
                        ...emptyActiveRooms.map(room => ({
                            text: room.room_name,
                            onPress: async () => {
                                const success = await executePurchaseFlow(itemType, itemId, itemCost, itemName, action, action_past_tense, room.active_room_id, undefined);
                                resolve(success);
                            }
                        })),
                        {
                            ...
                            style: "cancel",
                            onPress: () => {
                                console.log("Cat adoption cancelled at room selection.");
                                resolve(false);
                            }
                        }
                    ]
                ...
                Alert.alert(
                    ...
                    [
                        ...availableCats.map(cat => ({
                            text: cat.cat_name,
                            onPress: async () => {
                                const success = await executePurchaseFlow(itemType, itemId, itemCost, itemName, action, action_past_tense, undefined, cat.active_cat_id);
                                resolve(success);
                            }
                        })),
                        {
                            ...
                            style: "cancel",
                            onPress: () => {
                                console.log("Toy purchase cancelled at cat selection.");
                                resolve(false);
                            }
                        }
                    ]
            ...
            return await executePurchaseFlow(itemType, itemId, itemCost, itemName, action, action_past_tense, undefined, undefined);
        }
    };
